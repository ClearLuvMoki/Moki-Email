...
function mergeContacts(contacts: any[], email: string): Partial<ContactType>[] {
	const arr =
		[...contacts]
			.filter((item) => item.address !== email)
			.filter((c) => !!c.address) || [];

	return arr
		.reduce((acc, cur) => {
			const exists = acc.some(
				(item: any) => item.address === cur.address && item.name === cur.name,
			);
...
export class MailClientManager {
	...
	async checkMail() {
		...
		if (boxes.find((item) => item.specialUse === EmailBoxEnum.Inbox)?.key) {
			this.lock = await this.client.getMailboxLock(
				(boxes || []).find((item) => item.specialUse === EmailBoxEnum.Inbox)
					?.key!,
			);
		}
	}
	...
	async initMails(mailbox: string) {
		try {
			...
			const toFetch = (
				(remoteUids || []).filter((uid) => !existingUids.has(uid)) || []
			).sort((a, b) => b - a);
			...
			for (let i = 0; i < toFetch.length; i++) {
				...
				for await (const msg of this.client.fetch(
					...
					const isRead =
						Array.from(msg?.flags || []).includes("\\Seen") || false;
