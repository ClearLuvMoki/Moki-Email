    ...
    getMailList(data: MailListDto) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (data?.box) {
                    qb.andWhere('mail.boxPath = :box', { box: data.box })
                }
                if (data?.unRead) {
                    qb.andWhere('mail.isRead = :isRead', { isRead: !data.unRead })
                }
                if (data?.keyword) {
                ...
                const mailList = await Promise.allSettled(
                    mails.map(async item => {
                        const source: any = await MailToolManager.transformMailContent(item.source)
                        return {
                            ...item,
                ...
                return resolve({
                    data:
                        (mailList.filter(item => item.status === 'fulfilled') as any).map((item: any) => item.value) ||
                        [],
                    total: total,
                    pageNo: pageNo,
                    pageSize: pageSize,
