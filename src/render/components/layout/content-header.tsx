...
const RenderItem = ({ label, name, email }: { label: string; name: string; email: string }) => {
    return (
        <div className="flex">
...
const ContentHeader = () => {
    ...
    return (
        <div className="flex p-4 justify-between border-b border-b-zinc-200 h-[100px]">
            <div className="flex items-start gap-4">
                <Avatar className="border">
                    <AvatarImage src={`https://unavatar.io/${selectMail?.sender?.[0]?.email}`} />
                ...
                <div>
                    ...
                    />
                    <RenderItem label="To" name={''} email={selectMail?.to?.[0]?.email!} />
                </div>
            </div>
            <div>
                {selectMail?.date && (
